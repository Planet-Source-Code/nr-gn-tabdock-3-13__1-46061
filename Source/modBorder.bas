...
Public Sub DrawVSNetButtonBorder(hdc As Long, buttonRect As RECT, Align As tdAlignProperty)
  ...
  Dim tJunk     As POINTAPI
    ...
    rgbColor = TranslateColor(vbWhite)
    hMyPen = CreatePen(PS_SOLID, 1, rgbColor)
    
    ' Initialize misc variables
    ' select border pen
    hOldPen = SelectObject(hdc, hMyPen)
    
    ' draw color around the border
    Ret = MoveToEx(hdc, buttonRect.Right, buttonRect.Top, tJunk)
    Ret = LineTo(hdc, buttonRect.Left, buttonRect.Top)
    Ret = LineTo(hdc, buttonRect.Left, buttonRect.Bottom)
    ...
    rgbColor = TranslateColor(vbBlack)
    hMyPen = CreatePen(PS_SOLID, 1, rgbColor)
    ...
    Ret = LineTo(hdc, buttonRect.Right, buttonRect.Bottom)
    Ret = LineTo(hdc, buttonRect.Right, buttonRect.Top)
...
Public Sub DrawVSNetButtonBorderUnpinned(hdc As Long, buttonRect As RECT, Align As tdAlignProperty)
  ...
  Dim tJunk     As POINTAPI
    ...
    ' draw color around the border
    Ret = MoveToEx(hdc, buttonRect.Left, buttonRect.Top, tJunk)
'    Ret = LineTo(hdc, buttonRect.Left, buttonRect.Top)
    Ret = LineTo(hdc, buttonRect.Left, buttonRect.Bottom)
    Ret = SelectObject(hdc, hOldPen)
    Ret = DeleteObject(hMyPen)
