...
Private m_bUseITDockMoveEvents                    As Boolean
...
Err_AddForm:
    ...
    If Err.Number = 13 And Me.UseITDockMoveEvents = True Then
        Err.Raise Description:="Unexpected Error: " & vbCrLf & _
            "You have choosen to use the ITDockMoveEvents but have not implemented in your form ( " & Item.Caption & " ) code." & vbCrLf & _
            "Please Implement the ITDockMoveEvents interface on your forms.", _
            Number:=Err.Number, Source:=constSource
    Else
...
Public Property Let UseITDockMoveEvents(bln As Boolean)
Attribute UseITDockMoveEvents.VB_Description = "You Must Implement ITDockMoveEvents On Your Form For  TabDock To Call Directly To You Docked Form."

    m_bUseITDockMoveEvents = bln
    PropertyChanged "UseITDockMoveEvents"

End Property

Public Property Get UseITDockMoveEvents() As Boolean

    UseITDockMoveEvents = m_bUseITDockMoveEvents

End Property


Public Property Let PanelTopDockFormResize(bln As Boolean)
Attribute PanelTopDockFormResize.VB_Description = "Allows Resizing"
...
Attribute PanelLeftDockFormResize.VB_Description = "Allows Resizing"
...
Attribute PanelRightDockFormResize.VB_Description = "Allows Resizing"
...
Attribute PanelBottomDockFormResize.VB_Description = "Allows Resizing"
...
Attribute PanelResizeRight.VB_Description = "Allows Resizing"
...
Attribute PanelResizeLeft.VB_Description = "Allows Resizing"
...
Attribute PanelResizeTop.VB_Description = "Allows Resizing"
...
Attribute PanelResizeBottom.VB_Description = "Allows Resizing"
...
Public Property Let AutoCollapseBottom(Auto As Boolean)
    ...
    If Panels.Count > 0 Then
        If Not (Panels(tdAlignBottom).Expanded) Then
            Panels(tdAlignBottom).Panel_Expand
        Else
            Panels(tdAlignBottom).RefreshVSNETTabs
        End If
    End If
...
Public Property Let AutoCollapseLeft(Auto As Boolean)
    ...
    If Panels.Count > 0 Then
        If Not (Panels(tdAlignLeft).Expanded) Then
            Panels(tdAlignLeft).Panel_Expand
        Else
            Panels(tdAlignLeft).RefreshVSNETTabs
        End If
...
Public Property Let AutoCollapseRight(Auto As Boolean)
    ...
    If Panels.Count > 0 Then
        If Not (Panels(tdAlignRight).Expanded) Then
            Panels(tdAlignRight).Panel_Expand
        Else
            Panels(tdAlignRight).RefreshVSNETTabs
        End If
...
Public Property Let AutoCollapseTop(Auto As Boolean)
    ...
    If Panels.Count > 0 Then
        If Not (Panels(tdAlignTop).Expanded) Then
            Panels(tdAlignTop).Panel_Expand
        Else
            Panels(tdAlignTop).RefreshVSNETTabs
        End If
    End If
...
Attribute AutoExpand.VB_Description = "Auto Expand When Mouse Moves Over The Caption"
...
Attribute AutoShowCaptionOnCollapse.VB_Description = "Displays The Captions Of The forms When Collapsed"
...
Attribute CaptionStyle.VB_Description = "Docked Form Caption Style"
...
Public Property Let CaptionStyle(ByVal New_CaptionStyle As tdCaptionStyles)
  ...
  Dim f As TDockForm
    ...
    'Debug.Print "captionstyle"
    LockWindowUpdate Extender.Parent.hwnd
    For i = 1 To Panels.Count
        If m_CaptionStyle = tdCaptionVSNet Then
            'Panels(i).BackColor = BlendColor(vb3DHighlight, vb3DLight)
            Panels(i).AutoShowCaptionOnCollapse = True
            Panels(i).RefreshVSNETTabs
            Panels(i).Refresh
            For Each f In Panels(i).WindowList
                f.MaxButtonTag = f.HasMaxButton
                f.HasMaxButton = True
            Next
        Else
            Panels(i).BackColor = vbButtonFace
            For Each f In Panels(i).WindowList
                f.HasMaxButton = f.MaxButtonTag
            Next
        End If
...
Public Sub DockChange(formName As String, _
  ...
  Dim visCountCol As Integer
    ...
    If formIndex Then
        ...
        Me.DockedForms(formIndex).Panel.RefreshVSNETTabs
...
Public Sub GrabMain(MainFormHwnd As Long)
  ...
  Dim hWndMdi As Long
    ...
    AttachMessage Me, NewHWND, WM_ENABLE
    AttachMessage Me, NewHWND, WM_SIZE
...
Private Function ISubClass_WindowProc(ByVal hwnd As Long, _
    ...
    Select Case iMsg
      ...
      Case WM_ENABLE
        ...
        End If
      Case WM_SIZE
        If Not bFirstLoad And Me.Parent.WindowState = 0 Then
...
Attribute Persistant.VB_Description = "Automatically Saves The State"
...
Attribute Show.VB_Description = "Show the host panels and update docked forms"
  ...
  Dim f As TDockForm
    ...
    If CaptionStyle = tdCaptionVSNet Then
        For panelCount = 1 To Panels.Count
            'Panels(panelCount).BackColor = BlendColor(vb3DHighlight, vb3DLight)
            Panels(panelCount).AutoShowCaptionOnCollapse = True
            Panels(panelCount).RefreshVSNETTabs
            For Each f In Panels(panelCount).WindowList
                f.MaxButtonTag = f.HasMaxButton
                f.HasMaxButton = True
            Next
        Next panelCount
...
Attribute SmartSizing.VB_Description = "Reduces Screen Flicker When Sizing"
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
  ...
  'Load property values from storage
    ...
    UseITDockMoveEvents = PropBag.ReadProperty("UseITDockMoveEvents", False)
...
Private Sub UserControl_Terminate()
  ...
  Const constSource As String = m_constClassName & ".UserControl_Terminate"
    ...
    '*******************************************************************************
    ' this code has been commented out as it causes the form_load event to be fired
    '*******************************************************************************
    ...
    DetachMessage Me, NewHWND, WM_ENABLE
    DetachMessage Me, NewHWND, WM_SIZE
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
  ...
  'Write property values to storage
    ...
    Call PropBag.WriteProperty("UseITDockMoveEvents", m_bUseITDockMoveEvents, False)
