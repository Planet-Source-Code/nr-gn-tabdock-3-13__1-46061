...
Public Sub drawChevronButton(ByVal lHDC As Long, _
                              ...
                              Optional dPanel As TDockForm, _
                              Optional clickButton As Boolean = False, _
    ...
    Dim unpinned         As Boolean
    m_bOfficeXpStyle = False
    ...
    If clickButton Then
        ...
        If Active Then
            hPen = CreatePen(PS_SOLID, 1, BlendColor(vb3DHighlight, vbActiveTitleBar)) 'GetSysColor(vb3DHighlight And &H1F&))
        Else
    ...
    If Active Then
        hPen = CreatePen(PS_SOLID, 1, GetSysColor(vb3DHighlight And &H1F&))
    Else
        hPen = CreatePen(PS_SOLID, 1, TranslateColor(vbBlack))
    End If
    
    unpinned = False
    
    Select Case dPanel.Panel.Align
    Case tdAlignTop
        If dPanel.Panel.Parent.AutoCollapseTop Then
            unpinned = True
        End If
    Case tdAlignBottom
        If dPanel.Panel.Parent.AutoCollapseBottom Then
            unpinned = True
        End If
    Case tdAlignLeft
        If dPanel.Panel.Parent.AutoCollapseLeft Then
            unpinned = True
        End If
    Case tdAlignRight
        If dPanel.Panel.Parent.AutoCollapseRight Then
            unpinned = True
        End If
    End Select
    ...
    If unpinned Then
        ...
        'lX = m_tChevronR.Left
        'lY = m_tChevronR.Top
        lX = m_tChevronR.Left + (m_tChevronR.Right - m_tChevronR.Left - 10) / 2
        lY = m_tChevronR.Top + (m_tChevronR.Bottom - m_tChevronR.Top - 10) / 2
        
        MoveToEx lHDC, lX + 3, lY + 1, tJunk
        LineTo lHDC, lX + 3, lY + 8
        
        MoveToEx lHDC, lX + 3, lY + 2, tJunk
        LineTo lHDC, lX + 7, lY + 2
        LineTo lHDC, lX + 7, lY + 6
        LineTo lHDC, lX + 3, lY + 6
        LineTo lHDC, lX + 3, lY + 5
        LineTo lHDC, lX + 7, lY + 5
        
        MoveToEx lHDC, lX + 1, lY + 4, tJunk
        LineTo lHDC, lX + 3, lY + 4
    ...
    Else
        ...
        'lX = m_tChevronR.Left
        'lY = m_tChevronR.Top
        lX = m_tChevronR.Left + (m_tChevronR.Right - m_tChevronR.Left - 12) / 2
        lY = m_tChevronR.Top + (m_tChevronR.Bottom - m_tChevronR.Top - 11) / 2
        
        MoveToEx lHDC, lX + 3, lY + 2, tJunk
        LineTo lHDC, lX + 7, lY + 2
        LineTo lHDC, lX + 7, lY + 6
        LineTo lHDC, lX + 3, lY + 6
        LineTo lHDC, lX + 3, lY + 2
        
        MoveToEx lHDC, lX + 2, lY + 6, tJunk
        LineTo lHDC, lX + 9, lY + 6
        
        MoveToEx lHDC, lX + 6, lY + 2, tJunk
        LineTo lHDC, lX + 6, lY + 6
        
        MoveToEx lHDC, lX + 5, lY + 6, tJunk
        LineTo lHDC, lX + 5, lY + 9
...
Public Sub drawCloseButton(ByVal lHDC As Long, _
                            ...
                            Optional IsCollapsed As Boolean = False, _
                            Optional dPanel As TDockForm, _
                            Optional closeClick As Boolean = False, _
                            Optional Active As Boolean = False)
    ...
    If (m_bOfficeXpStyle) Then
        If (m_bCloseOver) Then
            If (m_bCloseDown) Then
                hbr = hBrTitle 'CreateSolidBrush(VSNetControlColor)
            Else '(m_bCloseDown) = FALSE/0
                hbr = hBrTitle 'CreateSolidBrush(VSNetSelectionColor)
            End If
            'FillRect lHDC, m_tCloseR, hbr
            If Active Then
                hPen = CreatePen(PS_SOLID, 1, GetSysColor(vbActiveTitleBar And &H1F&))
            Else
                hPen = CreatePen(PS_SOLID, 1, GetSysColor(vbButtonFace And &H1F&))
            End If
            hPenOld = SelectObject(lHDC, hPen)
            
            Rectangle lHDC, m_tCloseR.Left, m_tCloseR.Top, m_tCloseR.Right, m_tCloseR.Bottom
            SelectObject lHDC, hPenOld
            DeleteObject hPen
...
'        storedCaptionRect.Bottom = storedCaptionRect.Bottom + 10
            storedCaptionRect.Right = storedCaptionRect.Bottom - storedCaptionRect.Top - (getCaptionHeight * 2)
            storedCaptionRect.Bottom = storedCaptionRect.Bottom + 10
...
End Sub
Public Sub drawVSNETTab(captionRect As RECT, _
                          hdc As Long, _
                          captionText As String, _
                          bActive As Boolean, _
                          gradient As Boolean, _
    ...
    If captionOrientation = tdAlignLeft Or captionOrientation = tdAlignRight Then
        DrawIconEx hDCTemp, captionRect.Left + 2, captionRect.Top + 2, captionForm.Icon, 16, 16, 0&, 0&, DI_NORMAL
    Else
        DrawIconEx hDCTemp, captionRect.Left + 2, captionRect.Top + 2, captionForm.Icon, 16, 16, 0&, 0&, DI_NORMAL
    End If
    ...
    If bActive Then
        ...
        'move text a wee bit to the right
        If captionOrientation = tdAlignRight Or captionOrientation = tdAlignLeft Then
            storedCaptionRect.Right = storedCaptionRect.Bottom - storedCaptionRect.Top - (getCaptionHeight * 1.5)
            storedCaptionRect.Bottom = storedCaptionRect.Bottom + 10
            
            storedCaptionRect.Left = storedCaptionRect.Left + 2
            If captionOrientation = tdAlignRight Then
                storedCaptionRect.Right = storedCaptionRect.Left + storedCaptionRect.Right
            End If
            DrawText hDCTemp, captionText, Len(captionText), storedCaptionRect, DT_SINGLELINE Or DT_END_ELLIPSIS Or DT_BOTTOM
        Else 'NOT CAPTIONORIENTATION...
            storedCaptionRect.Bottom = storedCaptionRect.Bottom - 3
...
Public Sub drawVSNETTabUnpinned(captionRect As RECT, _
                          hdc As Long, _
                          captionText As String, _
                          bActive As Boolean, _
                          gradient As Boolean, _
                          Optional captionOrientation As tdAlignProperty, _
                          Optional captionForm As Form, _
                          Optional borderSurround As Boolean)
    ...
    If bActive Then
        hbr = CreateSolidBrush(TranslateColor(vbButtonFace)) '3DShadow))

        captionRect.Top = captionRect.Top - 4
        If captionOrientation = tdAlignTop Then
            captionRect.Bottom = captionRect.Bottom - 2
        Else
            captionRect.Bottom = captionRect.Bottom - 3
        End If

    Else
        hbr = CreateSolidBrush(BlendColor(vb3DHighlight, vb3DLight))
    End If
    ...
    If bActive Then
        DrawVSNetButtonBorderUnpinned hDCTemp, captionRect, captionOrientation
    End If
    ...
    DrawIconEx hDCTemp, storedCaptionRect.Left + 3, storedCaptionRect.Top + 1, captionForm.Icon, 16, 16, 0&, 0&, DI_NORMAL
    
    If Not bActive Then
        lineRect = captionRect
        hbr = CreateSolidBrush(TranslateColor(vb3DShadow))
        With lineRect
            .Left = .Right
            .Right = .Right + 1
    ...
    If True Then
        ...
        If bActive Then
            SetTextColor hDCTemp, TranslateColor(vbBlack)
        Else
            SetTextColor hDCTemp, TranslateColor(vb3DShadow)
        End If
        
        'move text a wee bit to the right
        storedCaptionRect.Bottom = storedCaptionRect.Bottom - 4
        storedCaptionRect.Left = storedCaptionRect.Left + 2
        storedCaptionRect.Right = storedCaptionRect.Right - 5
        storedCaptionRect.Left = storedCaptionRect.Left + getCaptionHeight
        DrawText hDCTemp, captionText, Len(captionText), storedCaptionRect, DT_SINGLELINE Or DT_END_ELLIPSIS Or IIf(borderSurround, DT_RIGHT, 0) Or DT_BOTTOM
...
Public Sub drawOfficeXP(captionRect As RECT, _
    ...
    If bActive Then
        ...
        Else 'BACTIVE = FALSE/0
        colorOutline = TranslateColor(vb3DShadow)
    ...
    SetPixel hDCTemp, captionRect.Left, captionRect.Top, colorInline
    SetPixel hDCTemp, captionRect.Right - 1, captionRect.Top, colorInline
    SetPixel hDCTemp, captionRect.Right - 1, captionRect.Bottom - 1, colorInline
    SetPixel hDCTemp, captionRect.Left, captionRect.Bottom - 1, colorInline
...
Public Sub drawVSNet(captionRect As RECT, _
    ...
    'move text a wee bit to the right
    If captionOrientation = tdAlignTop Or captionOrientation = tdAlignBottom Then
        storedCaptionRect.Right = storedCaptionRect.Bottom - (getCaptionHeight * 1)
        storedCaptionRect.Bottom = 8 + (captionForm.Height / Screen.TwipsPerPixelY)
        DrawText hDCTemp, captionText, Len(captionText), storedCaptionRect, DT_SINGLELINE Or DT_END_ELLIPSIS Or DT_BOTTOM
    Else 'NOT CAPTIONORIENTATION...
        storedCaptionRect.Left = storedCaptionRect.Left + 2
        storedCaptionRect.Right = storedCaptionRect.Right - (getCaptionHeight * 1)
        DrawText hDCTemp, captionText, Len(captionText), storedCaptionRect, DT_SINGLELINE Or DT_END_ELLIPSIS 'Or DT_HCENTER
    End If
...
Public Sub gradateColors(Colors() As Long, _
    ...
    If color1 < 0 Then
        color1 = GetSysColor(color1 And &H1F&)
    End If
    
    If color2 < 0 Then
        color2 = GetSysColor(color2 And &H1F&)
    End If
